...
def generate_data(n, m, m_star, s_common=0.2, s_idio=0.5):
    ...
    # Shock con asimmetria negativa (1 - Exp)
    xi_common = s_common * (1 - np.random.exponential(1.0, size=(n, 1)))
    xi_idio = s_idio * (1 - np.random.exponential(1.0, size=(n, m)))
...
df_results_parall.to_csv('df_results_run_100k_std_0.2_0.5_1-exp(1).csv')
