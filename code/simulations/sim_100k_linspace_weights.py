...
def generate_data(n, m, m_star, s_common=1.0, s_idio=0.5):
    
    mu = get_mu_vector_fast(m, m_star)
    
    weights = np.linspace(0.1, 0.9, m)
    ...
    # matrix broadcasting
    X = (weights * xi_common) + xi_idio + mu
...
df_results_parall.to_csv('df_results_run_100k_linspace_weights.csv')
