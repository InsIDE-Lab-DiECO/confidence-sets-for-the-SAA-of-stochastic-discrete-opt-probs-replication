...
def generate_data(n, m, m_star, s_common=0.2, s_idio=0.5):
    ...
    # shock with Exp(1) - 1
    xi_common = s_common * (np.random.exponential(1.0, size=(n, 1)) - 1)
    xi_idio = s_idio * (np.random.exponential(1.0, size=(n, m)) - 1)
...
df_results_parall.to_csv('df_results_run_100k_std_0.2_0.5_exp(1)-1.csv')
