# For server not to ff crash!
import os
# Force each task to stay on 1 thread so they dont fightt for the 32/128 slots
...
def generate_data(n, m, m_star, s_common=1, s_idio=1):
    ...
    w1 = np.linspace(0.04,0.25, m)
    w2 = np.linspace(0.25,0.04, m)
    # matrix broadcasting
    X = np.sqrt(w1) * xi_common + np.sqrt(w2) * xi_idio + mu
...
df_results_parall.to_csv('df_results_run_100k_04_025.csv')
